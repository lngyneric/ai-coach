...
The CLI defaults to `https://app.ai-shifu.cn`. Override with `--base-url <url>` on any command, or set `SHIFU_BASE_URL` in `.env`.

### Employee Login (Local / Self-Hosted)

For local deployments or AAD-enabled environments:

```bash
# Login with employee credentials
login --method employee --employee-no <id> --password <pwd> --base-url http://localhost:8080

# Or set SHIFU_BASE_URL once and omit --base-url on subsequent commands
export SHIFU_BASE_URL=http://localhost:8080
login --method employee --employee-no <id> --password <pwd>
```

For testing without the AAD server, set `AAD_BYPASS=1` in the backend environment.

To skip login entirely, set `--token` / `SHIFU_TOKEN` directly.
